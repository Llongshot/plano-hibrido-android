...
EXERCICIO_ANIMACAO = {ex["nome"]: ex["tipo"] for ex in EXERCICIOS_DATA}

class VideoPlayer(Widget):
    def __init__(self, video_url, video_id, **kwargs):
        ...
        # Fundo do thumbnail
        with self.thumbnail_container.canvas.before:
            Color(*CORES['fundo_claro'])
            self.thumb_rect = RoundedRectangle(
                size=self.thumbnail_container.size,
                pos=self.thumbnail_container.pos,
                radius=[10]
            )
        
        self.thumbnail_container.bind(
            size=lambda instance, value: setattr(self.thumb_rect, 'size', instance.size),
            pos=lambda instance, value: setattr(self.thumb_rect, 'pos', instance.pos)
        )
        ...
        with self.play_button.canvas.before:
            Color(*CORES['primaria'])
            self.play_circle = Ellipse(
                size=self.play_button.size,
                pos=self.play_button.pos
            )
        
        self.play_button.bind(
            size=lambda instance, value: setattr(self.play_circle, 'size', instance.size),
            pos=lambda instance, value: setattr(self.play_circle, 'pos', instance.pos)
        )
        ...
        video_title.bind(size=video_title.setter('text_size'))
        
        overlay_layout.add_widget(video_title)
        overlay_layout.add_widget(self.play_button)
        
        self.thumbnail_container.add_widget(overlay_layout)
        ...
        controls_layout.add_widget(btn_youtube)
        controls_layout.add_widget(btn_browser)
        
        self.layout.add_widget(self.thumbnail_container)
        self.layout.add_widget(controls_layout)
        
        self.add_widget(self.layout)
    
    def play_video(self, instance):
        # Simular reprodução (mudança visual)
        if not self.is_playing:
            ...
            anim.repeat = True
            anim.start(self.thumbnail_container)
            
            # Parar após 3 segundos (simulação)
            Clock.schedule_once(self.stop_video, 3)
        else:
            self.stop_video()
    
    def stop_video(self, dt=None):
        self.play_button.text = '▶'
        self.is_playing = False
        Animation.cancel_all(self.thumbnail_container)
        self.thumbnail_container.opacity = 1
    
    def open_youtube(self, instance):
        # Tentar abrir no app do YouTube (Android)
        if platform.system() == 'Linux':  # Android é baseado em Linux
            try:
                import subprocess
    ...
    def open_browser(self, instance):
        ...
        try:
            ...
            # Fallback: mostrar popup com URL
            self.show_url_popup()
    ...
    def show_url_popup(self):
        ...
        url_label = Label(
            text='Copie este link para ver o vídeo:',
            font_size='16sp',
            size_hint_y=0.3
        )
...
class SemanaScreen(Screen):
            ...
            if exercicios:
                for ex in exercicios:
                    ...
                    if ex_data:
                        ...
                        info_layout = BoxLayout(orientation='vertical', size_hint_x=0.4, spacing=5)
                        ...
                        # Botões
                        buttons_layout = BoxLayout(orientation='vertical', size_hint_x=0.4, spacing=5)
                        ...
                        btn_video.bind(on_press=lambda x, data=ex_data: self.mostrar_video(data))
                        
                        buttons_layout.add_widget(btn_iniciar)
                        buttons_layout.add_widget(btn_video)
                        
                        ex_container.add_widget(ex_animation)
                        ex_container.add_widget(info_layout)
                        ex_container.add_widget(buttons_layout)
    ...
    def mostrar_video(self, ex_data):
        ...
        content.add_widget(video_player)
        content.add_widget(btn_fechar)
...
class ExerciciosScreen(Screen):
    ...
    def build_layout(self):
        ...
        for ex in EXERCICIOS_DATA:
            ...
            left_container.add_widget(video_player)
